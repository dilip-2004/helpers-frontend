  ...
  const fetchHelperDetails = async () => {
    try {
      ...
      if (userData.addToCart.length > 1) {
        params = { helperIDs: userData.addToCart.join(',') };
      } else {
        params = { helperIDs: userData.addToCart[0] };
      }
            ...
            <div key={item._id} className="py-4 bg-slate-200 mt-2 rounded-lg px-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    className="text-gray-500 focus:outline-none bg-black px-4 rounded-2xl py-1 cursor-pointer"
                    onClick={() => deleteFromList(item._id)}
                  >
